...
const HeaderMain = () => {
  ...
  const favoriteCount = photos.filter((item) => item.isLike === true).length;
      ...
      {user ? (
        <div className="flex items-center gap-x-3">
          ...
          <span className="text-sm font-medium">
            Welcom back! <b>{user.fullname}</b>
          </span>
        </div>
      ) : (
        <span className="text-sm">Welcome</span>
      )}
      <div className="flex gap-x-5 justify-center mx-auto">
        <span className="w-8 relative">
          ...
          <span
           ...
           justify-center absolute top-0 right-0 translate-x-2/4 w-5 h-5 rounded-full bg-pink-600 text-white text-xs"
          >
            {cartItemCount || 0}
          </span>
        </span>
